    ...
    <div className="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? 'Edit User' : 'Tambah User'}>
        <form className="space-y-4 p-6" onSubmit={submitUser}>
          ...
          {!editingId && <Field label="Password"><div className="flex gap-2"><input required minLength="10" maxLength="72" type={showPassword ? 'text' : 'password'} className="min-w-0 flex-1 rounded-lg border p-2" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} /><button type="button" className="admin-btn-muted" onClick={() => setShowPassword(!showPassword)}>{showPassword ? 'Tutup' : 'Lihat'}</button></div><p className="mt-1 text-xs text-gray-500">Minimal 10 karakter serta mengandung huruf dan angka.</p></Field>}
      ...
      <Modal isOpen={Boolean(passwordUser)} onClose={() => setPasswordUser(null)} title={`Ganti Password ${passwordUser?.username || ''}`}>
        <form className="space-y-4 p-6" onSubmit={submitPassword}><Field label="Password Baru"><div className="flex gap-2"><input required minLength="10" maxLength="72" type={showPassword ? 'text' : 'password'} className="min-w-0 flex-1 rounded-lg border p-2" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} /><button type="button" className="admin-btn-muted" onClick={() => setShowPassword(!showPassword)}>{showPassword ? 'Tutup' : 'Lihat'}</button></div><p className="mt-1 text-xs text-gray-500">Minimal 10 karakter serta mengandung huruf dan angka.</p></Field><div className="flex justify-end gap-2"><button type="button" className="admin-btn-muted" onClick={() => setPasswordUser(null)}>Batal</button><button className="admin-btn-primary">Ganti Password</button></div></form>
      </Modal>
    </div>
  );
}
